...
export class DatabaseService {
    ...
    checkForSummoner(puuid: string) {
        return new Promise((resolve) => {
            this.http.get(this.apiUrl + '/summoners/' + puuid).subscribe(res => {
                var data: any;
                data = res;
                resolve(data.length);
            })
    ...
    putSummoner(summoner: Summoner) {
        return this.http.post(this.apiUrl + '/summoners', summoner);
    }
            ...
            for (const match in matches) {
                ...
                if (data.matchid) {
                    ...
                    const postPromise = new Promise((resolve) => {
                        this.http.post(this.apiUrl + '/matches', m).subscribe(res => {
                            var data: any;
                            data = res;
        ...
        return new Promise(async (resolve) => {
            ...
            try {
                data = await new Promise((resolve1) => {
                    this.http.get(this.apiUrl + '/RSDGames/' + puuid + '/' + queueId).subscribe(res => {
                        resolve1(res);
                    });
                })
                for (var x in data) {
