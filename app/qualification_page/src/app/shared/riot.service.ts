...
export class RiotService {
    ...
    getRiotSummonerData(region: string, username: string) {
        return this.http.get(this.apiUrl + '/online/summoner/' + region + '/' + username);
    }
    ...
    async updateMatchList(region: string, puuid: string): Promise<any> {
        try {
            ...
            const count = 100;
            var data: any;
            data = await this.http.get(this.apiUrl + '/online/matches/' + region + '/' + puuid + '/' + start + '/' + count).pipe(first()).toPromise();
            const matches = [];
            for (const m in data) {
                const match = await this.http.get(this.apiUrl + '/matches/' + puuid + '/' + data[m]).pipe(first()).toPromise();
                if (!match) {
                    matches.push(data[m]);
                }
            }
    ...
    async getMatchData(matchid: string, puuid: string, region: string) {
        return new Promise((resolve) => {
            this.http.get(this.apiUrl + '/online/mData/' + region + '/' + puuid + '/' + matchid).pipe(
                catchError(error => {
                    return of(false);
                })
            ).subscribe(res => {
    ...
    async getRankedData(region: string, id: string) {
        return new Promise((resolve) => {
            this.http.get(this.apiUrl + '/online/ranked/' + region + '/' + id).pipe(
                catchError(error => {
                    console.error(error);
                    resolve(false);
                    return of(false);
